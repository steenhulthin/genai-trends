...
- `google_trends_frequency`: source-specific count for the configured tertiary provider; the column name is kept for backward compatibility
...
- `social_media_frequency` currently maps to `Mastodon` hashtag timeline counts.
- `news_mentions_frequency` currently maps to `Guardian Open Platform`.
- `google_trends_frequency` currently maps to `Hacker News` story search counts.
- Multi-word tracked items are normalized into compact Mastodon hashtags, for example `system prompt -> #systemprompt`.
